...
async function pluginRequest(endpoint, method = 'GET', body = null) {
    ...
    console.log('[WhatsAppBridge] Request headers:', Object.keys(headers));
    ...
    try {
        ...
        console.log(`[WhatsAppBridge] Fetching: ${method} ${url}`);
        const response = await fetch(url, options);
        if (!response.ok) {
            const text = await response.text();
            console.error(`[WhatsAppBridge] HTTP ${response.status} from ${endpoint}:`, text.substring(0, 200));
            return null;
        }
